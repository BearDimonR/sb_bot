...
def get_parsed_categories():
    ...
    for row in record:  # Searching for all categories in every row
        if row[3]:
            categories_in_the_row = row[3].split(';')
            for category_in_the_row in categories_in_the_row:
                if category_in_the_row not in parsed_categories:  # Checking if we do not have this category in our array
                    parsed_categories.append(category_in_the_row)
